    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {activeTool === 'analyzer' && (
        <div className="bg-surface-container-low rounded-lg shadow-editorial p-6">
          <h2 className="text-2xl font-serif font-semibold text-on-surface mb-4">Excel Data Analyzer</h2>
          ...
          {analyzerStats && (
            <div className="space-y-4">
              <div className="bg-surface-container rounded-lg p-4">
                <h3 className="font-semibold text-on-surface mb-2">Overview</h3>
                <p className="text-on-surface">Rows: {analyzerStats.rowCount}</p>
                <p className="text-on-surface">Columns: {analyzerStats.columnCount}</p>
              </div>
              
              <div className="bg-surface-container rounded-lg p-4">
                <h3 className="font-semibold text-on-surface mb-3">Column Statistics</h3>
                <div className="space-y-3">
                  {Object.entries(analyzerStats.columns).map(([col, stats]: [string, any]) => (
                    <div key={col} className="border-l-4 border-primary pl-3">
                      <p className="font-medium text-on-surface">{col}</p>
                      <p className="text-sm text-on-surface/70">Type: {stats.type}</p>
                      <p className="text-sm text-on-surface/70">Unique: {stats.uniqueCount}</p>
                      <p className="text-sm text-on-surface/70">Nulls: {stats.nullCount}</p>
                      {stats.type === 'numeric' && (
                        <>
                          <p className="text-sm text-on-surface/70">Min: {stats.min}</p>
                          <p className="text-sm text-on-surface/70">Max: {stats.max}</p>
                          <p className="text-sm text-on-surface/70">Avg: {stats.avg.toFixed(2)}</p>
                        </>
                      )}
                    </div>
                  ))}
